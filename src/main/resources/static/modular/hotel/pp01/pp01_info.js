...
pp01InfoDlg.set = function (key, val) {
    this.pp01InfoData[key] = (typeof value == "undefined") ? $("#" + key).val() : val;
    return this;
};
...
pp01InfoDlg.collectData = function () {
    this.set('tbpp01Id').set('hotelGroupId').set('hotelId').set('configName').set('configCode').set('notice');
};
...
pp01InfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax_json(Feng.ctxPath + "/pp01/pp01Add", function (data) {
...
pp01InfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax_json(Feng.ctxPath + "/pp01/pp01Edit", function (data) {
        if(data.code == 403){
