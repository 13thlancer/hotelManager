...
$(function() {
    ...
    var table = new BSTable(pp01.id, "/pp01/list", defaultColunms);
...
//	layer提供了5种层类型。可传入的值有：0（信息框，默认）1（页面层）2（iframe层）3（加载层）4（tips层）
    var index = layer.open({
        ...
        fix: false, //不固定
        maxmin: true,
        content: Feng.ctxPath + '/pp01/toadd'
    });
...
pp01.modify = function () {
	if (this.check()) {
		var index = layer.open({
			...
			fix: false, //不固定
			maxmin: true,
			content: Feng.ctxPath + '/pp01/tomodify/' + pp01.seItem.tbpp01Id
		});
		this.layerIndex = index;
	}
};
...
pp01.del = function () {
    if (this.check()) {
        ...
        var operation = function(){
            var ajax = new $ax(Feng.ctxPath + "/pp01/delete", function (data) {
                Feng.success("删除成功!"); 
                pp01.table.refresh();
            }, function (data) {
